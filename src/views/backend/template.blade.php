...
@if(get_option('template') != $detail->path || !file_exists(resource_path('views/template/'.get_option('template').'/modules.php')))
...
<div class="row mt-4">
  ...
  <div class="list-group">
    ...
    <a class="list-group-item list-group-item-action {{! request('type') && $loop->first ? 'active':''}} {{request('type') && request('type')==Str::slug($row) ? 'active': ''}}" href="{{url()->current().'?type='.Str::slug($row)}}"><small>{{$row}}
  ...
  <div class="col-lg-4 mb-4">
              <div class="card">
                ...
                <div class="card-body">
                 ...
                 @if($row->path == get_option('template') && file_exists(resource_path('views/template/'.get_option('template').'/modules.php'))) <span class="text-success"> <i class="fa fa-check-circle" aria-hidden="true"></i> Diterapkan </span> @else <a class="btn btn-sm btn-outline-warning" href="#">Terapkan</a> @endif <a class="btn btn-sm btn-outline-info  pull-right " href="{{url()->current().'?type='.$row->type.'&select='.$row->id}}">Detail</a>
